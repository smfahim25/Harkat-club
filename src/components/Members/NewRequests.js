import React from 'react';
import { BiUserPlus, BiUserX } from 'react-icons/bi';
const NewRequests = () => {
    return (
        <div>
            <div className='flex justify-between mt-2'>
                <div className="py-1 w-full flex flex-row gap-2 items-center overflow-hidden rounded-full">
                    <span className="flex-shrink-0 inline-flex item-center border justify-center leading-none rounded-full">
                        <div className="avatar">
                            <div className="w-12 rounded-full border-2">
                                <img src="https://placeimg.com/192/192/people" alt='member pictures' />
                            </div>
                        </div>
                    </span>
                    <div className="flex-1 p-2">
                        <p className="text-md">Ad obcaecati eaque eum aut</p>
                    </div>
                    <div className='mr-5 border rounded-full px-2 bg-accent'>Primary Skill</div>
                </div>
                <div className='flex items-center cursor-pointer'>
                    <div className='text-3xl text-green-700 mr-3 hover:tooltip-accent hover:tooltip hover:tooltip-bottom' data-tip="Accept Request"><BiUserPlus /></div>
                    <div className='text-3xl text-red-700 mr-2 hover:tooltip-accent hover:tooltip hover:tooltip-bottom' data-tip="Delete Request"><BiUserX /></div>
                </div>
            </div>
        </div>
    );
};

export default NewRequests;