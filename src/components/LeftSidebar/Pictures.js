...
const Pictures = () => {
    ...
    return (
        <div className='mt-10 keep_scrolling h-[720px] 2xl:w-[800px] w-full'>
            {
                isLoading ? <div className='flex flex-col justify-center items-center mt-40'><PicturesLoading /> </div> : <div className='grid grid-cols-3 gap-4'> {club_media?.results?.map(club_image => club_image?.img !== null ? <Picture key={club_image.media_id
